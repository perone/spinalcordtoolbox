...
def main():
    ...
    if GM:
        ...
        for root, dirs, files in os.walk(input_folder):
            for dir in dirs:
                if dir.find("t2s") == -1 and dir.find("t2")== -1 and dir.find("mt") == -1 :
                    dirs_name.append(dir)
        print dirs_name
        for iter, dir in enumerate(dirs_name):
            for root, dirs, files in os.walk(input_folder + '/' + dir):
                for file in files:
        ...
        if not GM :
            for iter in range(0,len(list_data) / 2 - 1):
                ...
                if cpu_number != 0:
                    ...
                    for i in range(1, nz):
                        worker_slice_and_crop(folder_path + '/' + str(list_data[iter * 2]),
                                              folder_path + '/' + str(list_data[iter * 2 + 1]), fname_size_x,
                                              fname_size_y, v, i, center_seg)
        if GM:
            for iter in range(0, len(list_data) / 3 - 1):
    ...
    if nbre_slice != 0 and nbre_wrap != 0 :
        ...
        if '-i' in arguments:
            ...
            if GM:
                fname_src, fname_src_seg, fname_src_GM = random_list(folder_path, list_data, nbre_wrap, GM)
            else:
                fname_src, fname_src_seg = random_list(folder_path, list_data, nbre_wrap, 0)
            if verbose:
        ...
        if '-d' in arguments:
            ...
            start_time = time.time()
            if GM:
                fname_dest, fname_dest_seg, fname_dest_GM = random_list(folder_path, list_data, nbre_wrap, GM)
            else:
                fname_dest, fname_dest_seg = random_list(folder_path, list_data, nbre_wrap, 0)
            if verbose:
                elapsed_time = time.time() - start_time
        ...
        if '-t' in arguments:
            ...
            start_time = time.time()
            if GM:
                fname_im, fname_im_seg, fname_im_GM = random_list(folder_path, list_data, nbre_slice * nbre_wrap, GM)
            else:
                fname_im, fname_im_seg = random_list(folder_path, list_data, nbre_slice * nbre_wrap, GM)
            if verbose:
                elapsed_time = time.time() - start_time
        ...
        if GM:
            ...
            arguments_dest = [(fname_dest[iter], fname_dest_seg[iter], v, fname_src_GM[iter]) for iter in range(0, nbre_wrap)]
            arguments_im = [(fname_im[iter], fname_im_seg[iter], v, fname_src_GM[iter]) for iter in range(0, nbre_wrap*nbre_slice)]
        if not GM:
        ...
        if cpu_number != 0:
            ...
            for iter in range(0, nbre_wrap):
                if GM:
                    ...
                    arguments_dest = (fname_src[iter], fname_src_seg[iter], v, fname_dest_GM[iter])
...
# ==========================================================================================
def random_list(folder_path, list_data, nw, GM):
    list=[]
    list_seg=[]
    list_GM = []
    if not GM:
    ...
    if GM :
        for iter in range(0, nw):
            ...
            fname_src_seg = folder_path + '/' + str(list_data[random_index * 3 + 2])
            list.append(fname_src)
            list_seg.append(fname_src_seg)
            list_GM.append(fname_src_seg)
        return list, list_seg, list_GM
...
def crop_segmentation(fname, fname_seg, v, fname_GM):
    ...
    if fname_GM == None:
        ...
        if np.all(data_seg[:,:,0] == np.zeros((nx,ny))) or np.all(data_seg[:,:,nz] == np.zeros((nx,ny))):
            i = 1
...
def worker_random_slice(arguments_worker):
    ...
    try:
        ...
        if not fname_GM:
            dirs_name = file
        # get the number of the slice in the source image and crop it
        if not sct.check_folder_exist("Slices_" + dirs_name, verbose=0):
            os.makedirs("Slices_" + dirs_name)
        nx, ny, nz, nt, px, py, pz, pt = Image(fname).dim
        ...
        if not os.path.isfile(im):
            ...
            sct.run("sct_crop_image -i " + fname_GM + '/' + file_seg + ext_seg + " -dim 2 -start " + str(slice_z) + " -end " + str(slice_z) + " -o " + current_path + '/' + "Slices_" + dirs_name + '/' + file + "_slice_" + str(slice_z) + "_seg.nii.gz", verbose=v)
        ...
        im_GM = "Slices_" + dirs_name + '/'+ file + "_crop_" + str(nbre_im) + "_seg_o0.nii.gz"

        return im, im_seg, im_GM, nbre_im
    except KeyboardInterrupt:
        return
